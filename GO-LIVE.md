...
- [ ] **Configure status-callback webhook URL** in Twilio Console → Messaging → Toll-Free Verifications → Settings → Status Callback URL = `https://us-central1-plumenexus-prod.cloudfunctions.net/twilioStatusWebhook`. Without this the Admin status card won't auto-update (the `releaseTenantSMS` polling fallback can be added if Twilio doesn't reliably webhook).
