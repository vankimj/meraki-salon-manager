...
export default function FeedbackModal({ onClose }) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.45)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 300 }}
      ...
      <div style={{ background: '#fff', borderRadius: 16, width: '92%', maxWidth: 400, boxShadow: '0 20px 60px rgba(0,0,0,.25)', overflow: 'hidden' }}>
            ...
            <>
              ...
              <div style={{ display: 'flex', gap: 8, marginBottom: 14 }}>
                {[['bug', '🐛 Bug'], ['idea', '💡 Idea']].map(([val, label]) => (
                  <button key={val} onClick={() => setType(val)} style={{
                    ...
                    border: `1px solid ${type === val ? '#3D95CE' : '#d8d8d8'}`,
                    background: type === val ? '#EBF5FF' : '#fafafa',
                    color: type === val ? '#1a5f8a' : '#666',
              ...
              <div style={{ marginBottom: 14 }}>
                ...
                <textarea
                  ...
                  style={{ width: '100%', fontFamily: 'inherit', fontSize: 13, border: `1px solid ${text.trim() ? '#d8d8d8' : '#fca5a5'}`, borderRadius: 8, padding: '8px 12px', resize: 'vertical', outline: 'none', boxSizing: 'border-box', lineHeight: 1.5 }}
                />
              </div>

              <div style={{ display: 'flex', gap: 8 }}>
                <button onClick={onClose} style={btnStyle('#e8e8e8', '#333')}>Cancel</button>
